    ...
    <basic-container>
      ...
      >
        ...
        <template slot-scope="scope" slot="businessStatus">
          <el-tag :type="scope.row.businessStatus == '已关闭' ? 'info' : 'danger'">{{
            scope.row.businessStatus
          }}</el-tag>
        </template>
        <template slot-scope="scope" slot="execStatus">
          <el-tag :type="scope.row.execStatus == '已关闭' ? 'info' : 'danger'">{{
            scope.row.execStatus
          }}</el-tag>
        </template>
        <template slot-scope="scope" slot="settleStatus">
          <el-tag :type="scope.row.settleStatus == '已关闭' ? 'info' : 'danger'">{{
            scope.row.settleStatus
          }}</el-tag>
        </template>
        <template slot-scope="scope" slot="businessReason">
          <span>{{ getReason(scope.row.businessReason) }}</span>
        </template>
        <template slot-scope="scope" slot="execReason">
          <span>{{ getReason(scope.row.execReason) }}</span>
        </template>
        <template slot-scope="scope" slot="settleReason">
          <span>{{ getReason(scope.row.settleReason) }}</span>
        </template>
      </avue-crud>
    </basic-container>
...
import {
  ...
  closeOrder,
  openOrder
} from "@/api/basic/ordertotal";
import { remote } from "@/api/admin/dict";

import { tableOption } from "@/const/crud/basic/ordertotal";
...
export default {
  ...
  methods: {
    ...
    handleSubmit(event) {
      ...
      this.isOpen = false;
      //   if (event.offText) {
      //     openOrder({ id: event.id, reason: event.offText });
      //   } else {
      //     closeOrder(event.id);
      //   }
    },
