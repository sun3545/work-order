<template>
  <div class="app-container pull-auto ordersettle">
    <basic-container>
      <avue-crud
        ref="crud"
        :page="page"
      ...
      >
        <template slot="menuLeft">
          <el-button
            ...
            @click="handleAdd"
            size="small"
            v-if="permissions.generator_ordersettle_add"
            >新 增</el-button
          >
          <br /><br />
        </template>
        <template slot-scope="scope" slot="menu">
          <el-button
            v-if="permissions.generator_ordersettle_open"
            type="success"
            size="mini"
            icon="el-icon-open"
            plain
          ...
          >
          <el-button
            v-if="permissions.generator_ordersettle_close"
            type="warning"
            size="mini"
          ...
          >
          <el-button
            v-if="permissions.generator_ordersettle_down"
            icon="el-icon-check"
            size="mini"
            plain
          ...
          <el-button
            type="primary"
            v-if="permissions.generator_ordersettle_edit"
            icon="el-icon-edit"
            size="mini"
            plain
          ...
          <el-button
            type="danger"
            v-if="permissions.generator_ordersettle_del"
            icon="el-icon-delete"
            size="mini"
            plain
        ...
        <div class="ordera_customer">
          <template slot="customerId" slot-scope="scope">
            <el-tag>{{ scope.row }}</el-tag>
          </template>
        </div> -->
...
import {
  ...
  closeOrder,
  openOrder
} from "@/api/basic/ordersettle";
import { remote } from "@/api/admin/dict";
import { tableOption } from "@/const/crud/basic/ordersettle";
  ...
  methods: {
    ...
    async handleSubmit(event) {
      ...
      if (event.open.offText) {
        await openOrder({ id: event.open.id, reason: event.open.offText });
      } else {
    ...
    searchChange(params, done) {
      //     if (params.settleTime) {
      //     params.searchDate = params.settleTime;
      //     params.settleTime = undefined;
      //   }
    ...
    handleSave: function(row, done) {
      console.log(234567890, row);
      addObj(row).then(data => {
        console.log(343434, row, data);
