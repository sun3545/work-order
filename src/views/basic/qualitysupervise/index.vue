<template>
  <div class="app-container pull-auto supervise">
    <basic-container>
      <avue-crud
        ref="crud"
        :page="page"
      ...
      >
        <template slot="menuLeft">
          <el-button
            ...
            @click="handleAdd"
            size="small"
            v-if="permissions.generator_qualitysupervise_add"
            >新 增</el-button
          >
          <br /><br />
        </template>
        <template slot-scope="scope" slot="menu">
          <el-button
            type="success"
            v-if="permissions.generator_qualitysupervise_finish"
          ...
          <el-button
            type="primary"
            v-if="permissions.generator_qualitysupervise_edit"
            icon="el-icon-edit"
            size="mini"
            plain
          ...
          <el-button
            type="danger"
            v-if="permissions.generator_qualitysupervise_del"
            icon="el-icon-delete"
            size="mini"
            plain
...
import {
  ...
  delObj,
  getFinish
} from "@/api/basic/qualitysupervise";
import { getListObj } from "@/api/basic/qualitydept";
import { tableOption } from "@/const/crud/basic/qualitysupervise";
import { mapGetters } from "vuex";
export default {
  name: "qualitysupervise",
  components: {
    BulletFrame
  },
  data() {
  ...
  created() {
    this.getList();
    fetchList({ deptId: this.form.deptId }).then(res => {
      let data = res;
      this.dic = data.data.data.records;
    });
  },
  mounted: function() {},
  computed: {
